function(input, output, session) {
  ...
  Type_2 <- reactive({
    req(dosage_forcast_2())
    unique(dosage_forcast_2()[["Type"]])
  })
  
  output$type_ui_2 <- renderUI({
    req(Type_2())
    selectInput("type_2", "Please select the types", 
                choices = c("All Types", Type_2()), 
                multiple = TRUE, selected = "All Types")
  })
  
  filtered_forcast_2 <- reactive({
    req(dosage_forcast_2())
    d_filtered <- dosage_forcast_2()
    
    if (!("All Types" %in% input$type_2)) {
      d_filtered <- d_filtered %>% filter(Type %in% input$type_2)
    }
  ...
  forcast_target_Net <- eventReactive(input$calcButton_2, {
    ...
    d_forcast_filtered <- filtered_forcast_2() %>%
      ...
      group_by(Manufacturer, Medicine, Dosage, Country) %>%
      summarise(
    ...
    result <- d_forcast_filtered %>%
      full_join(d_sales_filtered, by = c("Manufacturer", "Medicine", "Dosage", "Country")) %>%
      mutate(
  ...
  output$forecast_card <- renderUI({
    ...
    value_box(
      ...
      value = scales::dollar(total_forecast, accuracy = 1),
  ...
  output$actual_card <- renderUI({
    ...
    value_box(
      ...
      value = scales::dollar(total_actual, accuracy = 1),
  ...
  output$plot_Manufacturer_Forcast_Net <- renderPlotly({
    ...
    data_to_plot <- data_plot_Manufacturer_Forcast_Net()
    
    num_manufacturers <- nrow(data_to_plot)
    sorted_palette <- generate_palette(num_manufacturers, c("#DB6400", "#272121"))
    ...
    p <- plot_ly(
      ...
      layout(
        title = "Total Net by Manufacturer",
        xaxis = list(title = "", categoryorder = "array", categoryarray = ~Manufacturer, tickangle = -45),
        yaxis = list(title = "Total Net"),
        margin = list(b = 150)
      )
  ...
  output$plot_Medicine_Forcast_Net <- renderPlotly({
    ...
    data_to_plot <- data_plot_Medicine_Forcast_Net()
    
    num_medicines <- nrow(data_to_plot)
    sorted_palette <- generate_palette(num_medicines, c("#DB6400", "#272121"))
    ...
    p <- plot_ly(
      ...
      layout(
        title = "Total Net by Medicine",
        xaxis = list(title = "", categoryorder = "array", categoryarray = ~Medicine, tickangle = -45),
        yaxis = list(title = "Total Net"),
        margin = list(b = 150)
      )
